...
func Parse(dirName string) RawParsingResults {
	name2club := make(map[string]int64)

	clubs := make([]RawClub, 0)
	dancers := make([]RawDancer, 0)
	dancerClubs := make([]RawDancerClub, 0)
	competitions := make([]RawCompetition, 0)
	jnjCompetitions := make([]RawCompetition, 0)
	nominations := make([]RawNomination, 0)
	jnjNominations := make([]RawNomination, 0)
	compResults := make([]RawCompetitionResult, 0)
	jnjResults := make([]RawCompetitionResult, 0)
	...
	allNominations := make([]RawNomination, 0, len(nominations)+len(jnjNominations))
...
type ByCompId []RawCompetitionResult
...
func fixJnjNominationIds(results []RawCompetitionResult, jnjResults []RawCompetitionResult, nominations []RawNomination) []RawCompetitionResult {
	allResults := make([]RawCompetitionResult, 0, len(results)+len(jnjResults))
	allResults = append(allResults, results...)
	allResults = append(allResults, jnjResults...)

	compIdToNominations := make(map[int64][]RawNomination)
	for i := range nominations {
		compId := nominations[i].CompetitionID
		foundNoms, ok := compIdToNominations[compId]
		if !ok {
			foundNoms = make([]RawNomination, 0)
		}
...
func crutchFixResult(result *RawCompetitionResult) {
	if result.CompetitionID == 18 && (result.DancerID == 822 || result.DancerID == 782) && result.Result == "A1/1" {
		result.AllPlacesMinClass = "B"
	}
}

func findNomination(result RawCompetitionResult, nominations []RawNomination) RawNomination {
	var suitableNominations []RawNomination
...
func isSuitable(result RawCompetitionResult, nomination RawNomination) bool {
...
func resultType(result RawCompetitionResult) string {
	if !result.IsJNJ {
		return "CLASSIC"
	}
...
func fixJnjResultsCompetitionIds(results []RawCompetitionResult, new2old map[int64]int64) []RawCompetitionResult {
	for i := range results {
...
func generateNominationIds(nominations []RawNomination) {
	var id int64 = 1
	for i := range nominations {
		nominations[i].ID = id
		id++
...
func fixJnjResults(results []RawCompetitionResult) []RawCompetitionResult {
	for i := range results {
...
func fixJnjResult(result *RawCompetitionResult) *RawCompetitionResult {
	res := result.Result
	s := res
	s = doCleanJnj(s)
...
func fixJnjNominationCompetitionIds(nominations []RawNomination, new2old map[int64]int64) []RawNomination {
	for i := range nominations {
...
func fixJnjNominations(nominations []RawNomination) []RawNomination {
	for i := range nominations {
...
func fixJnjNomination(nomination *RawNomination) *RawNomination {
	s := nomination.Value
	s = doCleanJnj(s)
...
func fixJnjCompetitionIds(jnj []RawCompetition, site2oldCompId map[string]int64, new2old map[int64]int64) []RawCompetition {
	for i := range jnj {
...
func fixCompResults(results []RawCompetitionResult) []RawCompetitionResult {
	newResults := make([]RawCompetitionResult, 0, len(results))
	for i := range results {
...
func fixResult(result *RawCompetitionResult) *RawCompetitionResult {
	//(Д-А30-32)C1/19+5
	//(С-Б6)C4/9+1
	//E4/21+3
...
func fixNominations(nominations []RawNomination) []RawNomination {

	newNominations := make([]RawNomination, 0, len(nominations))
	for i := range nominations {
	...
	newNominations = append(newNominations, []RawNomination{
		{
			Type:          "CLASSIC",
			CompetitionID: 247,
			FemaleCount:   2,
...
func fixNomination(nomination *RawNomination) *RawNomination {
...
func fixCompetitions(competitions []RawCompetition) []RawCompetition {
...
func fixDancers(dancers []RawDancer) []RawDancer {
	...
	dancers = append(dancers, RawDancer{
		ID:        -10,
		Code:      "-00010",
		Name:      "Елена",
		Surname:   "Заварухина",
...
func fixClubs(clubs []RawClub) []RawClub {
	maxClubId := findMaxClubId(clubs)
	clubs = append(clubs, RawClub{ID: 0, Name: "самост."})
	clubs = append(clubs, RawClub{ID: maxClubId + 1, Name: "Magnit"})
	clubs = append(clubs, RawClub{ID: maxClubId + 2, Name: "Intensity (г.Иваново)"})
	clubs = append(clubs, RawClub{ID: maxClubId + 3, Name: "Мартэ"})
	clubs = append(clubs, RawClub{ID: maxClubId + 4, Name: "Kids Office"})
	clubs = append(clubs, RawClub{ID: maxClubId + 4, Name: "Jumping Cats (г.Екатеринбург)"})
	clubs = append(clubs, RawClub{ID: maxClubId + 4, Name: "Движение (г.Новосибирск)"})

	return clubs
}

func findMaxClubId(clubs []RawClub) int64 {
...
func fixDancerClubs(original []RawDancerClub, name2club map[string]int64) []RawDancerClub {
	dancerClubs := make([]RawDancerClub, 0, len(original)*2)
...
func generateDancerClubs(names []string, name2club map[string]int64, original RawDancerClub) []RawDancerClub {
	if len(names) == 1 {
	...
	for _, name := range names {
		...
		dancerClub := RawDancerClub{ClubId: club, DancerID: original.DancerID, ClubNames: name}
...
func fillName2Club(name2club map[string]int64, clubs []RawClub) {
	for _, club := range clubs {
